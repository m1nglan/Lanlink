...
#define RTASR_USE_TLS            (0)          /*!< 0=ws(不加密,固件小);1=wss 需 esp-tls 且注意大固件可能影响 WiFi 射频 */
...
/*!
 * 讯飞实时语音转写客户端驱动(默认 ws:// 不加密,避免引入 TLS/证书包导致固件膨胀)
 * 使用示例:
 *   RtAsr asr;
...
class RtAsr {
public:
    esp_err_t init(void);                                   /*!< SNTP 同步时间→生成签名→ws 握手→启动 reader */
